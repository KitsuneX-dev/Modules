...
class SCDK(KitsuneModule):
    name = "SCDK"
    description = "K | Module for downloading songs from SoundCloud."
    author = "Fixyres"
    version = "1.0.0"
    icon = "🎵"
    category = "tools"
    ...
    strings_ru = {
        "_cls_doc": "K | Модуль для скачивания песен с SoundCloud.",
    ...
    @command("sc", required=OWNER, aliases=["scd"])
    async def scd(self, event):
        '''(link) - download a song from SoundCloud.'''
        args = self.get_args(event)
        if not args:
            await answer(event, self.strings("no_args").format(prefix=self._get_prefix()))
            return

        m = re.search(r"(https?://(?:[a-zA-Z0-9-]+\.)?soundcloud\.com/[^\s]+)", args)
        if not m:
            await answer(event, self.strings("not_found"))
            return

        msg = await answer(event, self.strings("downloading"))
        ...
        try:
            ...
            await answer(msg, self.strings("not_found"))
