...
Команды:
    ...
    .gpromt  [текст]     — задать роль/характер ИИ (системный промт)
...
class GeminiKModule(KitsuneModule):
    ...
    strings_ru = {
        ...
        "sdk_missing":     "⚠️ <b>Нужна библиотека <code>google-genai</code>.</b>\n<i>При первой загрузке модуля Kitsune ставит её автоматически (по <code>pip_requires</code>). Если сообщение осталось — загрузи модуль заново: ответь на файл модуля командой <code>.lm</code>.</i>\n<i>Не помогло — установи вручную: <code>.sh pip install google-genai</code></i>",
        "quota_error": (
        ...
        "quota_zero": (
            "🚫 <b>Модель <code>{}</code> недоступна на бесплатном тарифе (limit: 0).</b>\n"
            "Google убрал free-tier доступ для части моделей. Это не связано с твоим ключом.\n\n"
            "<b>Что делать:</b>\n"
            "• Переключись на бесплатную модель: <code>.gmodel gemini-2.5-flash</code>\n"
            "• Или включи биллинг: <a href='https://aistudio.google.com/app/billing'>тарифный план</a>"
        ),
        "region_error": (
        ...
        "prompt_show": (
            "🎭 <b>Текущий системный промт:</b>\n<blockquote>{}</blockquote>\n\n"
            "<i>Задать: </i><code>.gpromt текст роли</code>\n"
            "<i>Сбросить: </i><code>.gpromt clear</code>"
        ),
        "prompt_empty": (
            "🎭 <b>Системный промт не задан.</b>\n"
            "<i>ИИ отвечает без заданной роли/характера.</i>\n\n"
            "<i>Задать: </i><code>.gpromt ты дружелюбный ассистент по имени Kitsune</code>"
        ),
        "prompt_set":      "🎭 <b>Системный промт установлен.</b>\n<i>Длина: {} симв.</i>\n<blockquote>{}</blockquote>",
        "prompt_cleared":  "🧹 <b>Системный промт сброшен.</b>\n<i>ИИ снова отвечает без заданной роли.</i>",
        "prompt_too_long": "❗️ <b>Промт слишком длинный.</b>\n<i>Максимум {} символов, у тебя — {}.</i>",
    }
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.config = ModuleConfig(
            ...
            ConfigValue(
                ...
                doc="Модель Gemini (напр. gemini-2.5-flash, gemini-2.5-pro).",
            ...
            ConfigValue(
                ...
                validator=String(max_len=MAX_PROMPT_LEN),
            ),
            ConfigValue(
                "max_history",
                default=10,
    ...
    def _build_client(self, api_key: str):
        ...
        http_kwargs: dict = {"timeout": GEMINI_TIMEOUT * 1000}
        if proxy:
    ...
    def _classify_error(self, exc: Exception, model_name: str) -> tuple[str, bool]:
        ...
        if is_quota:
            if "limit: 0" in low or "'limit': 0" in low or '"limit": 0' in low:
                return self.strings("quota_zero").format(escape_html(model_name)), False
            return self.strings("quota_error"), True

        is_region = (
            "user location" in low
        ...
        if is_auth:
            return self.strings("invalid_api_key"), True

        return self.strings("api_error").format(escape_html(msg[:500])), True
    ...
    async def _call_gemini(self, parts: list, history: list) -> tuple[str, bool]:
        ...
        model_name = self._cfg("model_name", "gemini-2.5-flash")
        ...
        for key in keys:
            ...
            try:
                ...
                message, try_next = self._classify_error(e, model_name)
                last_err = message
                if not try_next:
                    return message, False
                logger.warning("GeminiK: key failed, trying next: %s", str(e)[:200])
                continue
            finally:
    ...
    @command("gpromt", required=OWNER, aliases=["gprompt", "grole"])
    async def gpromt_cmd(self, event) -> None:
        """[текст] — задать роль/характер ИИ (системный промт). clear — сбросить."""
        arg = (self.get_args(event) or "").strip()

        if not arg:
            current = (self._cfg("system_instruction", "") or "").strip()
            if not current:
                await event.edit(self.strings("prompt_empty"), parse_mode="html")
                return
            await event.edit(
                self.strings("prompt_show").format(escape_html(current)),
                parse_mode="html",
            )
            return

        if arg.lower() in ("clear", "reset", "off", "сброс", "очистить"):
            try:
                await self._save_cfg("system_instruction", "")
            except Exception as e:
                await event.edit(
                    self.strings("api_error").format(escape_html(str(e))),
                    parse_mode="html",
                ...
                return
            await event.edit(self.strings("prompt_cleared"), parse_mode="html")
            return

        if len(arg) > MAX_PROMPT_LEN:
            await event.edit(
                self.strings("prompt_too_long").format(MAX_PROMPT_LEN, len(arg)),
                parse_mode="html",
            )
            return

        try:
            await self._save_cfg("system_instruction", arg)
        except Exception as e:
            await event.edit(
                self.strings("api_error").format(escape_html(str(e))),
                parse_mode="html",
        ...
        await event.edit(
            self.strings("prompt_set").format(len(arg), escape_html(arg)),
            parse_mode="html",
        )
