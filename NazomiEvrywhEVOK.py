...
class _Waiter:
    ...
    def owns(self, msg_id):
        return self.tracked is not None and msg_id == self.tracked

    def wants_new(self, msg_id):
        if self.future.done():
            return False
        if self.tracked is not None and msg_id == self.tracked:
            return False
        return msg_id > self.after_id
...
class NazomiEvrywhEVOK(KitsuneModule):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.config = ModuleConfig(
            ConfigValue(
                ...
                default=False,
                doc=(
                    "Пересылать боту твои ответы (reply) на сообщение модуля. "
                    "Выключено — реплеи и цитаты остаются только в чате, "
                    "для ввода текста используй кнопку «✍️ Написать»."
                ),
                validator=Boolean(),
            ),
        )
    ...
    def _reply_input_enabled(self) -> bool:
        config = getattr(self, "config", None)
        if config is None:
            return False
        try:
            return bool(config["reply_input"])
        except Exception:
            return False
    ...
    def _feed(self, msg) -> None:
        ...
        if not self.waiters:
            return
        self._dispatch(msg, sig, msg_id)

    def _dispatch(self, msg, sig, msg_id) -> None:
        owners = [waiter for waiter in tuple(self.waiters) if waiter.owns(msg_id)]
        if owners:
            for waiter in owners:
                if waiter.base_sig is None or sig != waiter.base_sig:
                    waiter.deliver(msg)
            return

        for waiter in tuple(self.waiters):
            if waiter.background or not waiter.wants_new(msg_id):
                continue
            waiter.deliver(msg)
            return
        ...
        for waiter in tuple(self.waiters):
            if not waiter.background or not waiter.wants_new(msg_id):
                continue
            if waiter.owner_key != self.active_key:
                continue
            waiter.deliver(msg)
            return
    ...
    def _new_waiter(self, tracked=None, after_id=0, base_sig=None, background=False, owner_key=None):
        waiter = _Waiter(
            self,
            tracked=tracked,
            after_id=after_id,
    ...
    def _sync_target(self, session, call):
        ...
        if target is None:
            return None
        iid = getattr(call, "inline_message_id", None) if call is not None else None
        if iid and target.inline_message_id != iid:
            target.inline_message_id = iid
        elif not target.inline_message_id:
            resolved = self._resolve_inline_id(target.message_id)
            if resolved:
                target.inline_message_id = resolved
        return target

    def _resolve_inline_id(self, message_id):
        if not message_id:
            return None
        inline = self.get_inline()
        units = getattr(inline, "_units", None) if inline is not None else None
        if not units:
            return None
        for unit in units.values():
            tmsg = unit.get("telethon_msg")
            if tmsg is None or getattr(tmsg, "id", None) != message_id:
                continue
            iid = unit.get("inline_message_id")
            if iid and iid not in self._used_inline_ids():
                return iid
        return None
    ...
    async def _followup(self, key, msg_id, base_sig) -> None:
        ...
        try:
            while True:
                ...
                bound = self.bot_index.get(updated.id)
    ...
    async def evo_watcher(self, event) -> None:
        if not self._reply_input_enabled():
            return
    ...
    async def ns_cmd(self, event) -> None:
        ...
        known_iids = self._known_inline_ids()
        ...
        target = _EditTarget(
            ...
            form_message_id,
            self._find_inline_message_id(sent, known_iids),
        )
    ...
    def _used_inline_ids(self):
        used = set()
        for session in self.sessions.values():
            target = session.get("form")
            iid = getattr(target, "inline_message_id", None) if target is not None else None
            if iid:
                used.add(iid)
        return used

    def _find_inline_message_id(self, sent, known_iids=None):
        inline = self.get_inline()
        if inline is None:
            return None
        ...
        for unit in units.values():
            ...
            if tmsg is not None and getattr(tmsg, "id", None) == sent_id:
                ...
                if iid:
                    return iid
        blocked = set(self._used_inline_ids())
        if known_iids:
            blocked |= set(known_iids)
        for unit in units.values():
            iid = unit.get("inline_message_id")
            if iid and iid not in blocked:
                return iid
        return None
